...
if __name__ == "__main__":
    ...
    if PRED:
        ...
        pred_series_custom_btc = model_btc.predict(
            21,  # TODO: args
            series=series_custom_btc,
            num_samples=1
        )

        pred_series_custom_btc_inverse_trasform = transformer_btc.inverse_transform(pred_series_custom_btc)
        pred_series_custom_btc_inverse_trasform = pred_series_custom_btc_inverse_trasform.with_columns_renamed(
            ['0', '1'],
            ['price_btc', 'volume_btc']
        )

        # Visualization
        pred_series_custom_btc_inverse_trasform['price_btc'].plot(
            label=f"forecast BTC"
        )
        plt.legend()
